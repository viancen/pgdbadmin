        ...
        <div class="card border-term-accent/20 p-8 shadow-term-glow">
            <div class="mb-8">
                ...
                <h1 class="mt-2 font-mono text-2xl font-semibold text-term-text inline-flex items-center gap-3">
                    <img src="{{ asset('pgdb.png') }}" alt="pgdbadmin" class="h-8 w-8 shrink-0 object-contain" />
                ...
                <a href="{{ route('docs.connecting') }}" class="mt-2 inline-flex items-center gap-1 font-mono text-xs text-term-cyan hover:underline"><i data-lucide="external-link" class="w-3.5 h-3.5 shrink-0"></i> Direct & SSH tunnel</a>
            </div>
            ...
            <form action="{{ route('login') }}" method="post" class="space-y-4 font-mono">
                ...
                <button type="submit" class="btn-primary w-full font-mono inline-flex items-center justify-center gap-1.5"><i data-lucide="plug" class="w-4 h-4 shrink-0"></i> connect</button>
            </form>
        </div>
    </div>
</div>
